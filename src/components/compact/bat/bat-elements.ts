		...
		>
			<g>
				...
				>
					...
					<rect
						x="159"
						y="${compactMode ? '348' : '329.75'}"
						width="70"
						height="${compactMode ? '50' : '70'}"
						rx="${compactMode ? '7.5' : '10.5'}"
						ry="${compactMode ? '7.5' : '10.5'}"
						fill="none"
					...
					${renderText(
						...
						!config.battery.show_remaining_energy ||
							(compactMode && batteryCount === 2),
						!config.entities?.battery_status && !compactMode
							? 'remaining-energy'
				...
				>
					...
					>
						<svg
							id="bat_outter"
							x="${compactMode && batteryCount === 1 ? '212.5' : '232.5'}"
							y="325.5"
							width="78.75"
							height="78.75"
				...
				>
					...
					>
						...
						>
							<defs>
								<!-- Stable ID for battery2 gradient; Shadow DOM scoping prevents collisions across cards -->
						...
						>
							<defs>
								<!-- Stable ID for battery2 animated gradient; Shadow DOM scoping prevents collisions across cards -->
				...
				<svg id="battery_daily" style="overflow: visible;">
					<svg
						id="battery_daily_charge"
						style="overflow: visible;"
						x="${batteryCount === 2 ? '42%' : '0%'}"
						y="${batteryCount === 2 ? '-20%' : '0%'}"
					>
						${renderText(
							'daily_bat_charge',
							...
							'st3 left-align',
							batteryColour,
							config.battery.label_daily_chrg ||
						...
						${createTextWithPopup(
							...
							(e) =>
								Utils.handlePopup(e, config.entities.day_battery_charge_70),
							true,
						)}
					</svg>
					<svg
						id="battery_daily_discharge"
						style="overflow: visible;"
						x="${batteryCount === 2 ? '42%' : '0%'}"
						y="${batteryCount === 2 ? '-20%' : '0%'}"
					>
						...
						${createTextWithPopup(
							...
							(e) =>
								Utils.handlePopup(e, config.entities.day_battery_discharge_71),
							true,
						)}
					</svg>
				</svg>
