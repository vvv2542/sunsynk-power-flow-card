		...
		>
			...
			>
				<rect
					...
					ry="4.5"
					fill="none"
					stroke="${dynamicColourEssentialLoad5}"
					pointer-events="all"
			...
			>
				<rect
					...
					ry="4.5"
					fill="none"
					stroke="${dynamicColourEssentialLoad6}"
					pointer-events="all"
					display="${[5, 6].includes(additionalLoad) ? '' : 'none'}"
				/>
				${renderText(
			...
			${renderText(
				...
				130,
				([1, 2].includes(additionalLoad) && showAux) ||
			...
			<svg id="load1-flow">
				${renderPath(
					'es-line',
					config.wide
						? 'M 236 118 L 118 118 Q 108 118 108 128 L 108 162'
						: 'M 236 118 L 190 118 Q 180 118 180 128 L 180 162',
					true,
					dynamic_colour ? flowColour : loadColour,
					data.loadLineWidth,
				)}
			...
			<a
				...
				@click=${(e) =>
					config.load.navigate
						? Utils.handleNavigation(e, config.load.navigate)
						: null}
			>
				<svg
					...
					id="essen_aux"
					x="373.5"
					y="${data.essIconSize === 1 ? '82.5' : '78.5'}"
					width="77"
					height="77"
					viewBox="0 0 24 24"
				>
			...
			${createTextWithPopup(
				...
				showAux ? 160 : 41,
				[2, 4, 5, 6].includes(additionalLoad) &&
					data.stateEssentialLoad2.isValid(),
				'st3',
				dynamicColourEssentialLoad1,
				data.stateEssentialLoad1.toPowerString(auto_scale, decimalPlaces),
				(e) => Utils.handlePopup(e, config.entities.essential_load1),
			...
			${createTextWithPopup(
				...
				160,
				[4, 5, 6].includes(additionalLoad) &&
					data.stateEssentialLoad3.isValid(),
				'st3',
				dynamicColourEssentialLoad3,
			...
			${createTextWithPopup(
				...
				160,
				[4, 5, 6].includes(additionalLoad) &&
					data.stateEssentialLoad4.isValid(),
				'st3',
				dynamicColourEssentialLoad4,
